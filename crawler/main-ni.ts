...
async function crawl() {
    ...
    while (offset < queue.length) {
        ...
        if (got.ok) {            
            ...
            const type = text.indexOf("<h1>Empty D-block</h1>") !== -1 ? "A" : "P";

            const linkPattern = /<a href="(https:\/\/webarchive\.nationalarchives\.gov\.uk\/\d+\/http:\/\/www\.bbc\.co\.uk\/history\/domesday\/dblock\/NI-\d+-\d+)">(\w+)<\/a>/g;
